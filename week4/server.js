  ...
  if (req.url === "/api/credit-package" && req.method === "GET") {
    ...
    req.on("end", async () => {
      try {
        ...
        const skillRespo = await AppDataSource.getRepository("SKILL");
        const existSkill = await skillRespo.find({
          where: {
            name: data.name
          }
        });
        ...
        const newSkill = await skillRespo.create({
          name: data.name
        });
        const result = await skillRespo.save(newSkill);
